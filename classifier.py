...
def Tokenize(comment):
    ...
    comment = re.findall(r"[a-zA-Z]+", comment)
    words = []
    for i in range(len(comment)):
        if len(comment[i]) >= 2:
            words.append(comment[i])
...
def EmbeddingL2RegularizationUpdate(embedding_variable, net_input, learn_rate, l2_reg_val):
    ...
    # implement Y = Y - n d(R(y))/ dy
    # Y = Y - (2*L(R))
    # print(net_input.shape)
    net_input = tf.nn.l2_normalize(net_input, axis=1)
    net_input_T = tf.transpose(net_input)
    intr_val = tf.matmul(net_input_T, net_input)
    intr_val = tf.matmul(intr_val, embedding_variable)

    intr_val = tf.math.scalar_mul(learn_rate * 2 * l2_reg_val, intr_val)

    return embedding_variable.assign(embedding_variable - intr_val)
...
def EmbeddingL1RegularizationUpdate(embedding_variable, net_input, learn_rate, l1_reg_val):
    ...
    net_input = tf.nn.l2_normalize(net_input, axis=1)
    net_input_T = tf.transpose(net_input)
    intr_val = tf.math.sign(tf.matmul(net_input, embedding_variable))
    intr_val = tf.matmul(net_input_T, intr_val)
    intr_val = tf.math.scalar_mul(learn_rate * l1_reg_val, intr_val)
...
def SparseDropout(slice_x, keep_prob=0.3):
    ...
    """
    nonzero_indices = numpy.transpose(numpy.nonzero(slice_x))
    num_nz_indices = len(nonzero_indices)
    mat = numpy.zeros(num_nz_indices)
    new_size = int(num_nz_indices * (1 - keep_prob))
    mat[:new_size] = 1
    numpy.random.shuffle(mat)
    selected_indices = numpy.nonzero(mat)
    tbd_indices = numpy.transpose(nonzero_indices[selected_indices])
    slice_x[tbd_indices[0], tbd_indices[1]] = 0
...
def ComputeTSNE(embedding_matrix):
    ...
    """
    embeds_2d = manifold.TSNE(learning_rate=40, n_iter=545).fit_transform(embedding_matrix)
...
def VisualizeTSNE(sess):
    ...
    for w in class_to_words['positive']:
        i = TERM_INDEX[w]
        plt.scatter(tsne_embeddings[i, 0], tsne_embeddings[i, 1], c='b')
        plt.annotate(w, (tsne_embeddings[i, 0], tsne_embeddings[i, 1]))
    for w in class_to_words['location']:
        i = TERM_INDEX[w]
        plt.scatter(tsne_embeddings[i, 0], tsne_embeddings[i, 1], c='g')
        plt.annotate(w, (tsne_embeddings[i, 0], tsne_embeddings[i, 1]))
    for w in class_to_words['furniture']:
        i = TERM_INDEX[w]
        plt.scatter(tsne_embeddings[i, 0], tsne_embeddings[i, 1], c='r')
        plt.annotate(w, (tsne_embeddings[i, 0], tsne_embeddings[i, 1]))
    for w in class_to_words['negative']:
        i = TERM_INDEX[w]
        plt.scatter(tsne_embeddings[i, 0], tsne_embeddings[i, 1], c='orange')
        plt.annotate(w, (tsne_embeddings[i, 0], tsne_embeddings[i, 1]))

    plot_filename = "tsne_embeds"+sys.argv[2] +".pdf"
...
def BuildInferenceNetwork(x, l2_reg_val, is_training):
    ...
    with tf.variable_scope("fully_connected", reuse=tf.AUTO_REUSE):
        EMBEDDING_VAR = tf.get_variable("weights")
    return net
